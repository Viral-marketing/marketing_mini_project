...
def process_analysis(user, data):
    ...
    if data["about"] == "TOTAL_SPENDING":  # 소비일때는 WITHDRAW인 것만
        ...
        return "TOTAL_SPENDING(지출)과 TOTAL_INCOME(수입) 중 하나를 선택해주세요"

    # 제시한 기간에 맞는 거래내역이 없을 때 No data 출력
    if not queryset.exists():
        return "No data"

    # 각각의 instance들을 transaction_type과 transaction_amount로 이루어진 표 저장
    df = pd.DataFrame(list(queryset.values("transaction_type", "transaction_amount")))

    # 직렬화로 인해 string으로 형변환된 Decimal를 계산을 위해 float로 형변환
    # coerce는 숫자형으로 형변환이 안되는 string이나 특수문자를 NaN(Not a Number)로 처리
    # 500에러를 방지한다
    df["transaction_amount"] = pd.to_numeric(
        df["transaction_amount"], errors="coerce"
    ).astype(float)

    # 전체 instance들을 groupby로 transaction_type에 대한
    # transaction_amount의 총합을 표 형태로 저장
    ...
    Analysis.objects.create(
        ...
        description=(
            f"[{user['name']}]"
            f"[{data['period_start']} ~ {data['period_end']}] {data['about']}분석결과"
        ),
        result_image=None,  # 추후 추가할 기능으로 남겨둠
        result_json=summary.to_dict(),
        # JSONField로 인해 바로 파싱 할수 있도록 dict형식으로 저장
    )
    # 이메일 발송
    if (
        data["type"] == "CUSTOM"
    ):  # custom일때는 '요청하신'으로 시작하는 이메일 제목 전송
        if data["about"] == "TOTAL_SPENDING":
            # 이메일 전송할때 json.dumps로 직령화를 하는 과정에서
            # 전체 summary객체를 넘기면 typeerror가 발생함
